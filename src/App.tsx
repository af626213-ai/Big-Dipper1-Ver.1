...
// 音声を強制停止するユーティリティimport React, { useState, useEffect, useRef } from 'react';
import { Home, Play, CheckCircle, BookOpen, Volume2, Square, Mic, Headphones } from 'lucide-react';
import { ListeningStep } from './components/ListeningStep';
import { QuizStep } from './components/QuizStep';
import { VocabularyStep } from './components/VocabularyStep';
import { DictationStep } from './components/DictationStep';
...
const KeyPhrasesInternal = ({ items, rate, onNext }: { items: KeyPhrase[], rate: number, onNext: () => void }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in duration-500 font-pop text-left">
      <div className="text-center space-y-2">
        <div className="inline-block p-3 bg-lime-500 rounded-2xl text-white mb-2 shadow-md"><BookOpen size={32} /></div>
        <h2 className="text-3xl font-black text-slate-800 text-center">Key Phrases</h2>
      </div>
      <div className="bg-white rounded-[32px] p-8 shadow-xl border-4 border-lime-100 min-h-[250px] flex flex-col justify-center text-center space-y-6 relative">
        <button onClick={() => handleSpeak(currentItem.phrase)} className="absolute top-4 right-4 p-3 bg-lime-50 text-lime-600 rounded-full hover:bg-lime-100 transition-all">
          <Volume2 size={20} />
        </button>
        <h3 className="text-4xl font-black text-lime-600 tracking-tight">{currentItem.phrase}</h3>
        <p className="text-xl text-slate-700 leading-relaxed font-bold">{currentItem.explanation}</p>
      </div>
      <div className="flex gap-4">
        <button onClick={() => currentIndex > 0 && setCurrentIndex(c => c - 1)} disabled={currentIndex === 0} className={`flex-1 py-4 rounded-2xl font-bold ${currentIndex === 0 ? 'bg-slate-100 text-slate-300' : 'bg-white text-slate-600 border-2 border-slate-200 hover:border-lime-300'}`}>Back</button>
        <button onClick={handleNext} className="flex-[2] py-4 bg-lime-500 text-white font-bold rounded-2xl shadow-lg hover:bg-lime-600 active:scale-95 transition-all">{currentIndex < items.length - 1 ? 'Next Phrase' : 'Start Dictation'}</button>
      </div>
    </div>
  );
};
...
const OverlappingInternal = ({ script, rate, onNext }: { script: string, rate: number, onNext: () => void }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-500 font-pop">
      <div className="text-center space-y-2">
        <div className="inline-block p-3 bg-lime-500 rounded-2xl text-white mb-2 shadow-md"><Mic size={32} /></div>
        <h2 className="text-3xl font-black text-slate-800">Step 6: Overlapping</h2>
        <div className="bg-lime-50 border-2 border-lime-200 rounded-2xl p-4 mt-4 text-center">
          <p className="text-sm md:text-base font-bold text-lime-700 leading-relaxed">音声を聴きながらピッタリ重ねて音読しよう！</p>
        </div>
      </div>
      <div className="bg-white rounded-[32px] p-8 shadow-xl border-4 border-slate-100 relative text-left">
        <button onClick={handlePlay} className="absolute top-4 right-4 w-14 h-14 bg-lime-100 text-lime-600 rounded-full flex items-center justify-center hover:bg-lime-200 transition-all">
          {isPlaying ? <Square size={24} /> : <Volume2 size={24} />}
        </button>
        <p className="text-2xl text-slate-800 leading-relaxed pr-20 font-bold">{script}</p>
      </div>
      <button onClick={() => { stopSpeech(); onNext(); }} className="w-full py-5 bg-lime-500 text-white font-bold text-xl rounded-2xl shadow-lg hover:bg-lime-600 active:scale-95 transition-all">Go to Shadowing</button>
    </div>
  );
};
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-500 font-pop text-center">
      <div className="space-y-2">
        <div className="inline-block p-3 bg-lime-500 rounded-2xl text-white mb-2 shadow-md"><Headphones size={32} /></div>
        <h2 className="text-3xl font-black text-slate-800">Step 7: Shadowing</h2>
        <div className="bg-lime-50 border-2 border-lime-200 rounded-2xl p-4 mt-4 max-w-2xl mx-auto text-center">
          <p className="text-sm md:text-base font-bold text-lime-700 leading-relaxed">音声を影のように追いかけて音読しよう！</p>
        </div>
      </div>
      <div className="bg-white rounded-[32px] p-10 shadow-xl border-4 border-slate-100 flex flex-col items-center gap-6">
        <button onClick={handlePlay} className="w-28 h-28 bg-lime-500 text-white rounded-full flex items-center justify-center hover:bg-lime-600 shadow-md transform active:scale-95 transition-all">
          {isPlaying ? <Square size={40} /> : <Volume2 size={40} />}
        </button>
      </div>
      <button onClick={() => { stopSpeech(); onNext(); }} className="w-full py-5 bg-lime-500 text-white font-bold text-xl rounded-2xl shadow-lg hover:bg-lime-600 active:scale-95 transition-all">Complete Episode</button>
    </div>
  );
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .bg-orange-500 { background-color: #84cc16 !important; }
      .text-orange-600 { color: #4d7c0f !important; }
      .border-orange-500 { border-color: #84cc16 !important; }
      .hover\\:bg-orange-600:hover { background-color: #65a30d !important; }
      .bg-orange-50 { background-color: #f7fee7 !important; }
    `;
    document.head.appendChild(style);
  ...
  const renderLessonSection = (lessonNum: number, startId: number, endId: number) => (
    <section className="space-y-6 text-left">
      <div className="flex items-center gap-4 px-2">
        <h3 className="text-2xl font-black text-slate-800 flex items-center gap-3">
          <span className="bg-slate-700 text-white px-4 py-1.5 rounded-xl shadow-sm whitespace-nowrap">Lesson {lessonNum}</span>
        </h3>
        <div className="h-1 bg-slate-200 flex-1 rounded-full" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {courseData.episodes.filter(ep => ep.id >= startId && ep.id <= endId).map((ep, idx) => (
          <div key={ep.id} onClick={() => { stopSpeech(); setSelectedEpisode(ep); setCurrentStep('listening'); }} className="group relative bg-white rounded-[32px] p-6 border-4 border-slate-100 hover:border-lime-400 cursor-pointer transition-all shadow-sm hover:shadow-2xl hover:-translate-y-1 overflow-hidden">
            <div className="absolute -right-4 -bottom-6 text-9xl font-black text-slate-50 group-hover:text-lime-50 transition-colors pointer-events-none">{idx + 1}</div>
            <div className="relative z-10 flex items-center gap-5">
              <div className="w-14 h-14 rounded-2xl bg-lime-500 text-white flex items-center justify-center font-black text-xl shadow-lg group-hover:scale-110 transition-transform">P{idx + 1}</div>
              <div className="flex-1">
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Part {idx + 1}</p>
                <h3 className="text-lg md:text-xl font-black text-slate-800 leading-tight group-hover:text-lime-600 transition-colors">{ep.title}</h3>
              </div>
              <Play className="text-slate-300 group-hover:text-lime-500" size={20} fill="currentColor" />
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-pop">
      <header className="bg-white sticky top-0 z-50 border-b-4 border-slate-100 px-6 py-4 flex justify-between items-center shadow-sm">
        <button onClick={() => { stopSpeech(); setCurrentStep('menu'); }} className="p-2 bg-slate-100 hover:bg-lime-100 rounded-xl text-slate-800 transition-colors"><Home size={22} /></button>
        <button onClick={toggleBgm} className={`px-4 py-2 rounded-xl border-2 font-bold text-sm transition-all ${isBgmPlaying ? 'bg-lime-500 text-white border-lime-500 shadow-md' : 'bg-white text-slate-600 border-slate-200'}`}>{isBgmPlaying ? 'BGM ON' : 'BGM OFF'}</button>
      </header>

      {currentStep !== 'menu' && currentStep !== 'result' && (
        <nav className="bg-white border-b-2 border-slate-100 flex justify-center overflow-x-auto px-4">
          {(['listening', 'quiz', 'vocabulary', 'phrases', 'dictation', 'reading', 'overlapping', 'shadowing'] as const).map((step) => (
            <button key={step} onClick={() => { stopSpeech(); setCurrentStep(step); }} className={`py-3 px-4 text-[9px] font-black uppercase tracking-widest border-b-4 transition-all ${currentStep === step ? 'border-lime-500 text-lime-600' : 'border-transparent text-slate-400'}`}>{step}</button>
          ))}
        </nav>
      )}
      ...
      <main className="flex-1 p-6 text-center">
        <div className="max-w-4xl mx-auto">
          {currentStep === 'menu' && (
            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-1000">
              <div className="text-center py-16 bg-gradient-to-b from-lime-50 to-white rounded-[60px] border-4 border-lime-100 shadow-inner relative overflow-hidden">
                <div className="absolute top-0 left-0 w-32 h-32 bg-lime-200/20 rounded-full -translate-x-16 -translate-y-16" />
                <div className="absolute bottom-0 right-0 w-48 h-48 bg-slate-300/10 rounded-full translate-x-20 translate-y-20" />
                <h1 className="text-sm font-black text-slate-400 uppercase tracking-[0.5em] mb-4 relative z-10">The Ultimate Learning Method</h1>
                <h2 className="text-6xl md:text-7xl font-black text-slate-800 leading-none tracking-tighter relative z-10">English<br /><span className="text-lime-500">Navigator</span></h2>
                
                <div className="mt-8 relative z-10 flex flex-col items-center gap-3">
                  <div className="bg-white p-1.5 rounded-2xl shadow-md border-2 border-slate-100 flex gap-1">
                    {[0.6, 0.8, 1.0, 1.1].map((rate) => (
                      <button
                        key={rate}
                        onClick={() => setSpeechRate(rate)}
                        className={`px-4 py-2 min-w-[50px] rounded-xl font-black text-sm transition-all ${
                          speechRate === rate ? 'bg-lime-500 text-white' : 'text-slate-400 hover:bg-lime-50'
                        }`}
                      >
                        {rate.toFixed(1)}x
                      </button>
          ...
          {currentStep === 'result' && (
            <div className="max-w-md mx-auto text-center space-y-6 py-12 animate-in zoom-in duration-500">
              <CheckCircle size={80} className="text-lime-500 mx-auto" />
              <h2 className="text-4xl font-black text-slate-800">Lesson Cleared!</h2>
              <button onClick={() => setCurrentStep('menu')} className="w-full py-5 bg-lime-500 text-white font-bold text-xl rounded-2xl shadow-lg hover:bg-lime-600 transition-all">Back to Story List</button>
            </div>
          )}
        </div>
      </main>
